            ...
            ?>
                ...
                <tbody>
                    <tr>
                        ...
                        <td><?php echo $selected_data["order"] ?></td>

                        <?php
                        $gid = $selected_data["games_id"];
                        $image_rs = Database::search("SELECT * FROM `images` WHERE `games_id`='" . $selected_data["games_id"] . "' ");
                        $image_data = $image_rs->fetch_assoc();
                        ?>
                        <td> <img src="<?php echo $image_data["code"]; ?>" class="img-fluid rounded-start" style="height: 100px;width:100px;" /></td>
                        <?php
                        $games_rs = Database::search("SELECT * FROM `games` WHERE `id`='" . $selected_data["games_id"] . "' ");
                        $games_data = $games_rs->fetch_assoc();
                        ?>
                        <td><?php echo $games_data["title"] ?></td>
                        <td>$<?php echo $selected_data["total"] ?>.00</td>
