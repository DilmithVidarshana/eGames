...
if ($sort == 0) {
    ...
    if (!empty($txt)) {
        $query .= " WHERE `title` LIKE '%" . $txt . "%' AND `status`='1' ";
        $status = 1;
    }
    ...
    if (!empty($txt)) {
        $query .= " WHERE `title` LIKE '%" . $txt . "%' AND `status`='1' ORDER BY `price` DESC";
        $status = 1;
    }
} else if ($sort == 2) {
        ...
        <div class="row">
            ...
            <div class="card mt-3 mx-3 " style="width: 18rem; background-color:#000000;">
                ...
                <div class="card-body">
                    ...
                    <button class="btn btn-outline-danger" onclick='addToWatchlist(<?php echo $results_data["id"]; ?>)'><i class="bi bi-heart-fill"></i></button>
                    <button class="btn btn-outline-success" onclick="addToCart(<?php echo $results_data['id']; ?>);"><i class="bi bi-cart4"></i></button>
                    <a href='<?php echo "singleProductView.php?id=" . $results_data["id"]; ?>' class="col-12 btn btn-outline-primary mt-2">Buy Now</a>
                </div>
            </div>
        ...
        <ul class="pagination pagination-lg justify-content-center">
            <li class="page-item">
                <a class="page-link" <?php if ($pageno <= 1) {
                                        ...
                                        ?> onclick="advancedSearchh(<?php echo ($pageno - 1) ?>);" <?php
                                                                                                } ?> aria-label="Previous">
